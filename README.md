![docker image](https://github.com/Guillaume-Gaisne/oui-oasis/actions/workflows/docker-publish.yml/badge.svg)
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/Guillaume-Gaisne/oui-oasis.git
    cd oui-oasis
    ```
    ...
    ```sh
    curl-L -o oui-oasis.zip "https://github.com/Guillaume-Gaisne/oui-oasis/archive/main.zip"
    unzip oui-oasis.zip
    cd oui-oasis
    ```
...
your `docker-compose.yaml` with `ghcr.io/guillaume-gaisne/oui-oasis:main` for the services
...
By default, no plugins are included in this distribution. You can find a list of available NOMAD plugins [here](https://nomad-lab.eu/prod/v1/oasis/gui/search/plugins). For a list of official plugins provided by FAIRmat, please see [here](https://github.com/Guillaume-Gaisne/.github/blob/main/profile/README.md). For inspiration, you can also check the list of [plugins that are installed on the production NOMAD deployment hosted by FAIRmat](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-distro/-/raw/main/pyproject.toml?ref_type=heads).
...
```sh
docker pull ghcr.io/guillaume-gaisne/oui-oasis/jupyter:main
```
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/Guillaume-Gaisne/oui-oasis/manifests/main": unauthorized`
